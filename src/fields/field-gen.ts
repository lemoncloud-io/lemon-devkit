...
import type { DerivationInput, GenOptions, GenResult, RegistryEntry } from './types';
...
/**
 * migration 직후 사용하는 빈 registry.
 * - `migrate`가 먼저 써두면 rewrite 된 source가 즉시 compile 가능함.
 * - 이후 `gen`이 consumer project의 타입을 해석해서 concrete array로 교체한다.
 */
const STUB_BODY = `export const fieldKeys = {} as Record<
    string,
    <T extends object>() => Array<Extract<keyof T, string>>
>;\n`;

/** bootstrap stub 파일 내용을 생성 */
export const bootstrapStub = (): string => HEADER + STUB_BODY;
